...
/* Difference between MTU and required buffer size.
 * This accounts:
 * - possible 2 byte alignment,
 * - standard ethernet header,
 * - up to 8 bytes of VLAN or DSA tags,
 * - checksum */
#define FEC_FRAME_OVERHEAD	(2 + ETH_HLEN + 8 + ETH_FCS_LEN)
...
fec_enet_new_rxbdp(struct net_device *ndev, struct bufdesc *bdp, struct sk_buff *skb)
{
	...
	bdp->cbd_bufaddr = cpu_to_fec32(dma_map_single(&fep->pdev->dev, skb->data, fep->max_fl, DMA_FROM_DEVICE));
...
static int fec_change_mtu(struct net_device *ndev, int new_mtu)
{
	...
	if (new_mtu < 68 || new_mtu > fep->max_fl - FEC_FRAME_OVERHEAD)
		return -EINVAL;
	ndev->mtu = new_mtu;
	return 0;
}
