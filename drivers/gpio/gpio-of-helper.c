...
static DEVICE_ATTR(status, 0444, gpio_of_helper_show_status, NULL);
...
gpio_of_entry_create(struct gpio_of_helper_info *info, struct device_node *node)
{
	...
	if (of_property_read_bool(node, "input"))
		...
		dev_err(dev, "No valid node type (input/output) specified\n");
		err = -EINVAL;
		goto err_bad_node;
	}

	/* Next, the GPIO number (and descriptor). */
	err = of_get_named_gpio_flags(node, "gpio", 0, &gpio_flags);
	if (IS_ERR_VALUE(err)) {
		dev_err(dev, "Failed to get gpio property\n");
		goto err_bad_node;
	}
	gpio = err;

	if (!gpio_is_valid(gpio)) {
		dev_err(dev, "Skipping unavailable gpio %d\n", gpio);
		goto err_bad_node;
	}
	...
	/* Use existing name if already assigned.  Otherwise, parse. */
	if (desc->name && desc->name[0]) {
		name = desc->name;
		dev_info(dev, "Using existing name property '%s'\n", name);
	} else {
	...
	/* request the gpio */
	err = devm_gpio_request_one(dev, gpio, req_flags, "gpio-of-helper");
	if (err != 0) {
		dev_err(dev, "Failed to request gpio for '%s'\n", name);
		goto err_bad_node;
	}
	...
	spin_lock(&info->idr_lock);
	err = idr_alloc(&info->idr, entry, 0, 0, GFP_NOWAIT);
	if (err >= 0)
		entry->id = err;
	spin_unlock(&info->idr_lock);
	idr_preload_end();
	if (err < 0) {
		dev_err(dev, "Failed to idr_get_new() for '%s'\n", name);
		goto err_fail_idr;
	}

	dev_info(dev, "Allocated GPIO '%s' (%d) for export\n", name, entry->id);
...
static const struct dev_pm_ops gpio_of_helper_pm_ops = {
...
struct platform_driver gpio_of_helper_driver = {
	...
	.driver = {
		.name		= "gpio-of-helper",
		.of_match_table	= of_match_ptr(gpio_of_helper_of_match),
