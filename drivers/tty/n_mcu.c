...
static bool n_mcu_validate_checksum_crc16(const u8 const *data,
				   size_t len)
{
	u16 crc = 0xffff;
					    ...
					    size_t len)
{
	...
	/* calculate 8 bit 2s complement from buffer */
	for (i = 0; i < len; i++)
		checksum += data[i];
...
static int n_mcu_set_checksum(struct n_mcu_priv *priv, unsigned int type)
{
	int ret = 0;
	...
	case N_MCU_CHECKSUM_NONE:
		priv->add_checksum = NULL;
		priv->validate_checksum = NULL;
		break;
	case N_MCU_CHECKSUM_CRC16:
		priv->add_checksum = n_mcu_add_checksum_crc16;
		priv->validate_checksum = n_mcu_validate_checksum_crc16;
		break;
	case N_MCU_CHECKSUM_8B2C:
		priv->add_checksum = n_mcu_add_checksum_8b2c;
		priv->validate_checksum = n_mcu_validate_checksum_8b2c;
		break;
	default:
		ret = -EINVAL;
		break;
...
static void n_mcu_close(struct tty_struct *tty)
{
	...
	wake_up_interruptible(&n_mcu_priv.wait);
}
		...
		unsigned int cmd, unsigned long arg)
{
	...
	case N_MCU_SET_CHECKSUM_TYPE:
		return n_mcu_set_checksum(priv, arg);
